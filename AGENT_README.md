...
**How to Update the Flagship PDF**
`rzst-flagship-full.pdf` is a static, manually curated file — it is not generated by any script. It is served directly by GitHub Pages when a visitor clicks a download button. To release a new version:
1. Prepare and finalize the new PDF externally (e.g., in Adobe Acrobat).
2. Name the file `rzst-flagship-full.pdf` and place it in the repository root, overwriting the old file.
3. Commit and push to `main` — GitHub Pages will serve the new file automatically.

No build tools, no scripts, and no Node.js are involved in this workflow.
