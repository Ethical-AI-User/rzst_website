...
| [AUD-F02/F03 Proposed-Control Re-Test](aud-f02-f03-proposed-control-retest.md) | Deterministic specification evaluation; coherent proposed fixture routes without acceptance or runtime proof |
...
The audit evaluates documentation coherence only. It does not make a claim about a live Manus or Zapier configuration, actual model behavior, permissioning, data retention, endpoint response, workflow execution, or operational alert delivery. The AUD-F02/F03 re-test evaluates a **proposed** specification only; both findings remain open until an accepted patch is incorporated and re-tested. Any runtime claim requires separately authorized implementation testing.
